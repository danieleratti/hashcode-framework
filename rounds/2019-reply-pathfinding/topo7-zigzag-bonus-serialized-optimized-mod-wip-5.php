...
//$possibilities = false; //force avoid cache
if ($possibilities) {
...
function alignPossibilities()
{
    ...
    if ($officesDone <= 0) { // no offices done -> maximize profit the first iteration
        Log::out('no offices done');
        $km0 = 1; //maxProfit
        $kp0 = 1;
    ...
    if ($officesDone >= 0) { // some offices done -> maximize both profit and missing //TODO: mettere un >= anziché solo >
        Log::out('some offices done');
        $km0 = 0; //maxProfit
        $kp0 = 1;
    ...
    if ($remainingOffices <= 2) { // only 1 offices left -> maximize only missing [Tune this!]
        Log::out('last office');
        $km0 = 0; //maxProfit
        $kp0 = 1;
...
if ($skipRead) {
    ...
    for ($r = 0; $r < $map->rowCount; $r += $sampleSize) {
        ...
        for ($c = 0; $c < $map->colCount; $c += $sampleSize) {
            Log::out('R = ' . $r . '/' . $map->rowCount . ' && C = ' . $c, 1);
            /** @var PathMap $pathMap */
            ...
            if($map->cells[$r][$c]->cost <= 50) { //only very green cells
                foreach ($caches as $pathMap) {
