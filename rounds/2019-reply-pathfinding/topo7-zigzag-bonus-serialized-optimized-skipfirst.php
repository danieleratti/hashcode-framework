...
function alignPossibilities()
{
    ...
    if ($officesDone >= 0) { // some offices done -> maximize both profit and missing
    ...
    if ($remainingOffices <= 1) { // only 1 offices left -> maximize only missing
        Log::out('last office');
        $km0 = 0; //maxProfit
        $kp0 = 1;
    ...
    if ($nOffices == 1) {
        ...
        foreach($possibilities as $p) {
            if($kp == 0) {
                ...
                if($p['missingOrPositiveClientsCount'] != $firstMissingOrPositiveClientsCount) {
                    $pos = $kp;
                    break;
                }
            }
