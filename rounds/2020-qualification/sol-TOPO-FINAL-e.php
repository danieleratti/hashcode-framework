...
function fullAlignLibrary($libraryId)
{
    ...
    if ($library) {
        ...
        if ($takeDays > 0) {
            ...
            //$library->booksChunkedScore = pow($percentileChunkScore * 1, 1) / pow($library->signUpDuration, 1); //5090208
            
            //$library->booksChunkedScore = pow($booksChunkedScore * 1 + $percentileChunkScore * 1, 1) / pow($library->signUpDuration, 1); //5037852
            
            //$avanzoDivRemtime = $takeDays / ($countDays - $currentDay);
            //$library->booksChunkedScore = pow($booksChunkedScore * $library->shipsPerDay * 1, 0.71) / pow($library->signUpDuration, 1) * pow($avanzoDivRemtime, 8.9); //millyadapted [running con +...] //4865720
            
            //$library->booksChunkedScore = pow($percentileChunkScore * 1 + $booksChunkedScore/5, 1) / pow($library->signUpDuration, 1); //5046374
            
            //$library->booksChunkedScore = pow($percentileChunkScore * 1 + $booksChunkedScore*($currentDay/$countDays) * 1, 1) / pow($library->signUpDuration, 1); //5039725
            
            $library->booksChunkedScore = pow($percentileChunkScore * 1, 1.5) / pow($library->signUpDuration, 1); //running
