...
$kPow2 = 1.0;
$MOREDAYS = 1;
Cerberus::runClient(['fileName' => 'e', 'MOREDAYS' => 1]);
...
function switchBook($switch)
{
    ...
    $books[$switch['switch_book']]->unscanFirst($libFrom);
    $books[$switch['pop_book_to']]->unscanFirst($libTo);
...
// FAKE +MOREDAYS
for ($t = 0; $t < $countDays+$MOREDAYS; $t++) {
...
// Realign - MOREDAYS
$doneDays = 0;
foreach ($orderedSignuppedLibraries as $idSl => $sl) {
    if($doneDays+$sl->signUpDuration >= $countDays) {
        foreach($sl->scannedBooks as $sb) {
            $sb->unscanFirst($sl);
            $totalScore -= $sb->award;
        }
        unset($orderedSignuppedLibraries[$idSl]);
    }
    else {
        $doneDays += $sl->signUpDuration;
        $numBooks = ($countDays - $doneDays) * $sl->shipsPerDay;
        foreach($sl->scannedBooks as $sb) {
            $numBooks--;
            if($numBooks < 0) {
                $sb->unscanFirst($sl);
