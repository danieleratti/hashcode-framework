...
foreach(array_slice($COMBINATIONS[2], 0, 1000) as $c2) {
...
foreach(array_slice($COMBINATIONS[3], 0, 1000) as $c2) {
    $pizza1 = $PIZZAS[$c2['id1']];
    $pizza2 = $PIZZAS[$c2['id2']];
...
Log::out('Flatting');
$flatCombinations = array_merge($COMBINATIONS[2], $COMBINATIONS[3], $COMBINATIONS[4]);
ArrayUtils::array_double_keysort($flatCombinations, 'myscore', 'DESC', 'idsCount', 'ASC'); //DESC

Log::out('Assigning');
foreach($flatCombinations as $comb) {
    if($remainingTeams == 0) {
        Log::out('Teams ended');
        break;
    }
