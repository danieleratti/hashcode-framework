...
function getSimulation($inputSemaphores)
{
    ...
    $intersectionId2T2greenStreet[$heavyIntersectionId] = [];

    $remainingStreetsOnHeavy = [];
    foreach ($INTERSECTIONS[$heavyIntersectionId]->streetsIn as $streetIn) {
        if ($streetIn->stoppingCars > 0)
            $remainingStreetsOnHeavy[$streetIn->name] = true;
    }
    ...
    for ($T = 0; $T < $DURATION + 1; $T++) {
        ...
        foreach ($INTERSECTIONS as $intersection) {
            if ($intersection->id == $heavyIntersectionId && count($remainingStreetsOnHeavy) > 0 && !isset($intersectionId2T2greenStreet[$intersection->id][$T])) {
                $lastForcedStreetName = $intersectionId2T2greenStreet[$intersection->id][$T - 1];
                if(count($semaphoreQueues[$lastForcedStreetName]) > 0) {
                    $intersectionId2T2greenStreet[$intersection->id][$T] = $lastForcedStreetName;
                }
                else {
                    $unusedStreet2points = [];
                    foreach ($intersection->streetsIn as $streetIn) {
                        if (!$usedStreets[$streetIn->name] && count($semaphoreQueues[$streetIn->name]) > 0) {
                            ...
                            foreach ($semaphoreQueues[$streetIn->name] as $carId) {
                                if ($DURATION - $car2durationFromHeavyIntersection[$carId] > 160) // TUNE THIS
                                    $points += $car2durationFromHeavyIntersection[$carId];
                            }
                            $unusedStreet2points[] = ['streetName' => $streetIn->name, 'points' => $points];
                        }
                    }
                    ArrayUtils::array_keysort($unusedStreet2points, 'points', 'DESC');
                    $forcedStreetName = $unusedStreet2points[array_key_first($unusedStreet2points)]['streetName'];
                    if ($forcedStreetName) {
                        unset($remainingStreetsOnHeavy[$forcedStreetName]);
                        for ($i = 0; $i < count($semaphoreQueues[$forcedStreetName]); $i++) {
                            $intersectionId2T2greenStreet[$intersection->id][$T + $i] = $forcedStreetName;
                        }
                    } else {
                        $intersectionId2T2greenStreet[$intersection->id][$T] = $lastForcedStreetName;
                    }
            ...
            if (isset($intersectionId2T2greenStreet[$intersection->id])) {
                ...
                if (!$usedStreets[$greenStreetName] && $intersection->id != $heavyIntersectionId) {
                    //Log::out("$T) Int" . $intersection->id);
                    $unusedStreet2points = [];
                    foreach ($intersection->streetsIn as $streetIn) {
                        if (!$usedStreets[$streetIn->name] && count($semaphoreQueues[$streetIn->name]) > 0) {
                            if ($intersection->id == $heavyIntersectionId) {
                                // intersezione pesante
                                $points = 0;
                                foreach ($semaphoreQueues[$streetIn->name] as $carId) {
                                    if ($DURATION - $car2durationFromHeavyIntersection[$carId] > 160)
                                        $points += $car2durationFromHeavyIntersection[$carId];
                                }
                                $unusedStreet2points[] = ['streetName' => $streetIn->name, 'points' => $points];
                            } else {
                    ...
                    foreach ($unusedStreet2points as $unusedStreet) {
                        if ($unusedStreet['streetName'] == $greenStreetName) {
                            if ($intersection->id == $heavyIntersectionId) // <-- todo: vedere qui se si riesce a migliorare trySwitch per supportare i compounds!
                                Log::out("Fail to switch!");
                            break;
                        }
