...
$cycleMaxDuration = 5;
$OVERHEADQUEUE = 0;
Cerberus::runClient(['fileName' => 'c', 'bestCarsPerc' => 1.0, 'cycleMaxDuration' => 1]);
Autoupload::init();
...
function trySwitch(&$cfg, $T, $bestStreetInName, $usedStreets)
{
        ...
        foreach ($INTERSECTIONS as $intersection) {
            if (count($_config[$intersection->id]) > 0) {
                ...
                if (count($streetSemaphoreQueue[$streetName]) == 0 && !$usedStreets[$streetName]) {
                    ...
                    foreach ($intersection->streetsIn as $streetIn) { // poi ordinare per punteggi delle car in coda :)
                        if (!$usedStreets[$streetIn->name] && count($streetSemaphoreQueue[$streetIn->name]) > 0) {
                            ...
                            if($config[$intersection->id][$bestStreetInName] == 1 && $config[$intersection->id][$streetName] == 1) { // mod simple (only 1)
                                if (trySwitch($_config[$intersection->id], $T, $bestStreetInName, $usedStreets)) {
                                    $streetName = $_config[$intersection->id][$T % count($_config[$intersection->id])];
                                    break;
                                } else
                                    continue;
