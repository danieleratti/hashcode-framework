...
$cycleMaxDuration = 5;
$OVERHEADQUEUE = 0;
$deltaStartingTime = 5.0;
Cerberus::runClient(['fileName' => 'c', 'bestCarsPerc' => '1', 'cycleMaxDuration' => '1', 'deltaStartingTime' => 5]); // Cerberus::runClient(['fileName' => 'c', 'bestCarsPerc' => 1.0, 'cycleMaxDuration' => 1]);
Autoupload::init();
        ...
        foreach ($INTERSECTIONS as $intersection) {
            if (count($_config[$intersection->id]) > 0) {
                ...
                if (count($streetSemaphoreQueue[$streetName]) == 0 && !$usedStreets[$streetName]) {
                    ...
                    $bestStreetCountSemaphoreQueue = 0;
                    foreach ($intersection->streetsIn as $streetIn) {
                        if (!$usedStreets[$streetIn->name] && count($streetSemaphoreQueue[$streetIn->name]) > 0) { // TODO: Search the best!!! Priority order!
                            if(count($streetSemaphoreQueue[$streetIn->name]) > $bestStreetCountSemaphoreQueue) {
                                $bestStreetInName = $streetIn->name;
                                $bestStreetCountSemaphoreQueue = count($streetSemaphoreQueue[$streetIn->name]);
                            }
                        }
                    }
                    if($bestStreetInName) {
...
foreach ($initialStreetsWaitingTime as $k => $v) {
    $initialStreetsWaitingTime[$k] += $deltaStartingTime;
}
