...
$OVERHEADQUEUE = 0;
Cerberus::runClient(['fileName' => 'd', 'bestCarsPerc' => '1']); // Cerberus::runClient(['fileName' => 'd', 'bestCarsPerc' => 1.0]);
Autoupload::init();
        ...
        foreach ($INTERSECTIONS as $intersection) {
            if (count($_config[$intersection->id]) > 0) {
                ...
                if (count($streetSemaphoreQueue[$streetName]) == 0 && !$usedStreets[$streetName]) {
                    ...
                    foreach($intersection->streetsIn as $streetIn) { //TODO: Prendo la strada più interessante (?)
...
$configScore = getScore($configScore['config']);
Log::out("SCORE($fileName, $bestCarsPerc, $cycleMaxDuration) = {$configScore['score']} ({$configScore['bonus']} + {$configScore['early']})");
