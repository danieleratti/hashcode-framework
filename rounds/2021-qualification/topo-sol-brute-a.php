...
$cycleMaxDuration = 5;
$OVERHEADQUEUE = 5;
Cerberus::runClient(['fileName' => 'a', 'bestCarsPerc' => 1.0, 'cycleMaxDuration' => 5]);
Autoupload::init();
...
$int2sem = [];
$semaphoresCombinations = [];
$semaphoresSubCombinations = [];

// heating
foreach($INTERSECTIONS as $intersection) {
    foreach($intersection->streetsIn as $streetIn) {
        $int2sem[$intersection->id][] = $streetIn->name;
    }
}
/*
// sub
foreach($int2sem as $intersectionId => $streets) {
    if(count($streets) == 1) {
        $semaphoresSubCombinations[$intersectionId][] = [$streets[0] => 1];
    } else {
...
for($i=0;$i<7;$i++) {
    for($j=0;$j<7;$j++) {
        if($i != 0 || $j != 0) {
            $semaphores[1]["rue-d-athenes"] = $j;
            $semaphores[1]["rue-d-amsterdam"] = $i;
            $semaphoresComb[] = $semaphores;
        }
    }
}

foreach($semaphoresComb as $semaphores) {
