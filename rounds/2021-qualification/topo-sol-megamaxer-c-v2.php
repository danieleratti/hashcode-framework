...
$OVERHEADQUEUE = 0;
$kMultiLostScore = 1;
Cerberus::runClient(['fileName' => 'c', 'bestCarsPerc' => '1', 'cycleMaxDuration' => '1']); // Cerberus::runClient(['fileName' => 'c', 'bestCarsPerc' => 1.0, 'cycleMaxDuration' => 1]);
Autoupload::init();
...
function getSemaphores($streetsWaitingTime, $bestCarsPerc = 1.0, $cycleMaxDuration = 5)
{
    ...
    foreach ($CARS as $car) {
        ...
        $cars[] = ['car' => $car, 'maxPoints' => $maxScore];
        /*
        $maxPoints = $BONUS;
        $lostPoints = 0;
        foreach ($car->streets as $idx => $street) {
...
$fileManager->outputV2(getOutput($configScore['config']), '_c_score_' . $configScore['score']);
