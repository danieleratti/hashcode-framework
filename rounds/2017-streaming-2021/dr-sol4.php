...
function calculateBestScore($video, $startingCombination=[])
{
    /** @var Video $video */
    global $ENDPOINTS;
    global $VIDEOS;
    ...
    if(count($startingCombination) == 0)
        $possibleCachesCombinations = ArrayUtils::getAllCombinationsFlatLimited($possibleCacheIds, 1);
    else
    {
        $possibleCachesCombinations = [];
        foreach($possibleCacheIds as $cacheId) {
            if(!in_array($cacheId, $startingCombination))
                $possibleCachesCombinations = array_merge($startingCombination, [$cacheId]);
        }
    }
    ...
    foreach ($possibleCachesCombinations as $cachesCombination) {
        ...
        $score = $score / ($video->size * pow(count($cachesCombination),0.2)); // TUNING
        if ($score > $bestScore) {
            $bestScore = $score;
...
while (count($VIDEOS) > 0) {
    ...
    if(count($bestVideo->bestCaches) == 1)
        calculateBestScore($bestVideo, $bestVideo->bestCaches);
