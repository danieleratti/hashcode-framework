...
function calculateBestScore($video)
{
    /** @var Video $video */
    global $ENDPOINTS;
    global $VIDEOS;
    ...
    $possibleCachesCombinations = ArrayUtils::getAllCombinationsFlatLimited($possibleCacheIds, nMaxCachesPerVideo);
    ...
    foreach ($possibleCachesCombinations as $cachesCombination) {
        ...
        // se $score > $bestScore
        $score = $score / pow($video->size * count($cachesCombination), 2); // TUNING
        if ($score > $bestScore) {
            $bestScore = $score;
...
foreach ($VIDEOS as $videoId => $video) {
    ...
    //if($videoId%1 == 0)
    Log::out("Calculating best scores (" . $videoId . "/" . count($VIDEOS) . ")");
